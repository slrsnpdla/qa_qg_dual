...
from keras.layers import TimeDistributed as TimeDistributed
...
class Model(object):
    ...
    def _build_forward(self):
                ...
                with tf.variable_scope("emb_var"), tf.device("/cpu:0"):
                    if config.mode == 'train':
                        word_emb_mat = tf.get_variable("word_emb_mat", dtype='float',
                                                       shape=[VW, dw], initializer=get_initializer(config.emb_mat),
                                                       trainable=True)
                    else:
        ...
        with tf.variable_scope("prepro"):
            ...
            if config.share_lstm_weights:
                tf.get_variable_scope().reuse_variables()
                (fw_h, bw_h), ((_, fw_h_f), (_, bw_h_f)) = bidirectional_dynamic_rnn(cell_fw, cell_bw, xx, x_len, dtype='float', scope='u1')  # [N, M, JX, 2d]
                h = tf.concat(axis=3, values=[fw_h, bw_h])  # [N, M, JX, 2d]
            else:
                (fw_h, bw_h), ((_, fw_h_f), (_, bw_h_f)) = bidirectional_dynamic_rnn(cell_fw, cell_bw, xx, x_len, dtype='float', scope='h1')  # [N, M, JX, 2d]
                h = tf.concat(axis=3, values=[fw_h, bw_h])  # [N, M, JX, 2d]
        ...
        with tf.variable_scope("q_gen"):
            ...
            q_worthy = tf.reduce_sum(tf.to_int32(self.y), axis=2) # so we get probability distribution of answer-likely. (N, M)
            q_worthy = tf.expand_dims(tf.to_int32(tf.argmax(q_worthy, axis=1)), axis=1) # (N) -> (N, 1)
            q_worthy = tf.concat([tf.expand_dims(tf.range(0, N, dtype=tf.int32), axis=1), q_worthy], axis=1)
            # example : [0, 9], [1, 11], [2, 8], [3, 5], [4, 0], [5, 1] ...

            ss = tf.gather_nd(xx, q_worthy)
            syp = tf.to_float(tf.expand_dims(tf.gather_nd(self.y, q_worthy), axis=-1))
            syp2 = tf.to_float(tf.expand_dims(tf.gather_nd(self.y2, q_worthy), axis=-1))
            ...
            if layers > 1:
                cell_fw = rnn.MultiRNNCell(
                    [rnn.DropoutWrapper(rnn.GRUCell(qg_dim), input_keep_prob=config.input_keep_prob) for _ in range(layers)])
            ...
            s_outputs, s_states = tf.nn.bidirectional_dynamic_rnn(cell_fw, cell_bw, ss_with_ans, dtype=tf.float32)
            s_outputs = tf.concat(s_outputs, axis=2)
            if layers > 1:
                (s_states_fw, s_states_bw) = s_states
                # s_states = tf.reduce_sum([tf.reduce_sum(s_states_fw, axis=0), tf.reduce_sum(s_states_bw, axis=0)], axis=0)
                s_states = tf.concat([tf.reduce_sum(s_states_fw, axis=0), tf.reduce_sum(s_states_bw, axis=0)], axis=1)
            else:
                s_states = tf.concat(s_states, axis=1)

            start_tokens = tf.zeros([N], dtype=tf.int32)
            self.inp_q_with_GO = tf.concat([tf.expand_dims(start_tokens, axis=1), self.q], axis=1)
            # supervise if mode is train
            if config.mode == "train":
                emb_q = tf.nn.embedding_lookup(params=word_emb_mat, ids=self.inp_q_with_GO)
                #emb_q = tf.reshape(tf.tile(tf.expand_dims(emb_q, axis=1), [1, M, 1, 1]), (NM, JQ+1, dw))
                train_helper = seq2seq.TrainingHelper(emb_q, [JQ] * N)
            else:
                s_outputs = seq2seq.tile_batch(s_outputs, multiplier=beam_width)
                s_states = seq2seq.tile_batch(s_states, multiplier=beam_width)

            cell = rnn.DropoutWrapper(rnn.GRUCell(num_units=qg_dim * 2), input_keep_prob=config.input_keep_prob)
            attention_mechanism = seq2seq.BahdanauAttention(num_units=qg_dim * 2, memory=s_outputs)
            attn_cell = seq2seq.AttentionWrapper(cell, attention_mechanism,
                                                 attention_layer_size=qg_dim * 2,
                                                 output_attention=True, alignment_history=False)
            total_glove_vocab_size = 78878#72686
            out_cell = rnn.OutputProjectionWrapper(attn_cell, VW+total_glove_vocab_size)
            if config.mode == "train":
                decoder_initial_states = out_cell.zero_state(batch_size=N, dtype=tf.float32).clone(cell_state=s_states)
                decoder = seq2seq.BasicDecoder(cell=out_cell, helper=train_helper, initial_state=decoder_initial_states)
            else:
                decoder_initial_states = out_cell.zero_state(batch_size=N*beam_width, dtype=tf.float32).clone(cell_state=s_states)
                decoder = seq2seq.BeamSearchDecoder(cell=out_cell, embedding=word_emb_mat,
                                                    start_tokens=start_tokens, end_token=EOS_TOKEN,
                                                    initial_state=decoder_initial_states, beam_width=beam_width,
                                                    length_penalty_weight=0.0)
            outputs = seq2seq.dynamic_decode(decoder=decoder, maximum_iterations=JQ)
            if config.mode == "train":
                ...
                gen_q_prob = tf.nn.embedding_lookup(params=word_emb_mat, ids=outputs[0].predicted_ids[:, :, 0])
                gen_q_states = outputs[1]
    ...
    def _build_loss(self):
        config = self.config
        N = config.batch_size
        NM = config.batch_size * config.max_num_sents
        JX = tf.shape(self.x)[2]
        M = tf.shape(self.x)[1]
        JQ = tf.shape(self.q)[1]
        EOS_TOKEN = 1
        ...
        if config.wy:
            ...
            """
            corr_sent_idx = tf.reduce_sum(tf.to_int32(self.y), axis=2) # so we get probability distribution of answer-likely. (N, M)
            corr_sent_idx = tf.expand_dims(tf.to_int32(tf.argmax(corr_sent_idx, axis=1)), axis=1) # (N) -> (N, 1)
            corr_sent_idx = tf.concat([tf.expand_dims(tf.range(0, N, dtype=tf.int32), axis=1), corr_sent_idx], axis=1)
            # example : [0, 9], [1, 11], [2, 8], [3, 5], [4, 0], [5, 1] ...

            pos_gen_q_prob = tf.gather_nd(tf.reshape(self.gen_q_prob, (N, M, JQ, -1)), corr_sent_idx)
            pos_seq2seq_loss = seq2seq.sequence_loss(logits=pos_gen_q_prob, targets=self.q, weights=weights, average_across_batch=False) # N
            tot_seq2seq_loss = tf.reduce_mean(tf.reshape(seq2seq.sequence_loss(logits=self.gen_q_prob, targets=tf.reshape(tf.tile(tf.expand_dims(self.q, axis=1), [1, M, 1]), (N*M, JQ)),
                                                     weights=tf.reshape(tf.tile(tf.expand_dims(weights, axis=1), [1, M, 1]), (N*M, JQ)),
                                                     average_across_batch=False), (N, M)), axis=1) # N
            # negative sampling loss
            seq2seq_loss = pos_seq2seq_loss / tf.maximum(1.0, tot_seq2seq_loss - pos_seq2seq_loss)
            seq2seq_loss = tf.reduce_mean(seq2seq_loss) + tf.reduce_mean(pos_seq2seq_loss)
            """
            weights = tf.to_float(tf.not_equal(self.inp_q_with_GO[:, :-1], EOS_TOKEN))
            pos_gen_q_prob = tf.reshape(self.gen_q_prob, (N, JQ, -1))
            seq2seq_loss = seq2seq.sequence_loss(logits=pos_gen_q_prob, targets=self.q, weights=weights)
            seq2seq_loss = tf.where(tf.is_nan(seq2seq_loss), 1.0, tf.pow(2.0, seq2seq_loss))
        self.loss = tf.add_n(tf.get_collection('losses', scope=self.scope), name='loss')
        self.seq2seq_loss = tf.identity(seq2seq_loss, name='seq2seq_loss')

        tf.summary.scalar(self.loss.op.name, self.loss)
        tf.summary.scalar(self.seq2seq_loss.op.name, self.seq2seq_loss)

        tf.add_to_collection('ema/scalar', self.loss)
        tf.add_to_collection('ema/scalar', self.seq2seq_loss)
    ...
    def get_qg_loss(self):
        return self.seq2seq_loss

    def get_qg_sample(self):
        return self.gen_q
    ...
    def get_feed_dict(self, batch, is_train, supervised=True):
        ...
        x = np.zeros([N, M, JX], dtype='int32') # 1은 EOS_TOKEN을 의미
        cx = np.zeros([N, M, JX, W], dtype='int32')
        x_mask = np.zeros([N, M, JX], dtype='bool')
        q = np.ones([N, JQ], dtype='int32') # 1은 EOS_TOKEN을 의미
        ...
        def _get_word(word):
            ...
            return 3 # UNK이 [3]번에있음

        def _get_char(char):
            d = batch.shared['char2idx']
            if char in d:
                return d[char]
            return 1 # NULL, UNK 이므로 UNK를 의미.
