...
def read_data(config, data_type, ref, data_filter=None):
    ...
    if False:#if data_type == "train": #if not ref: # 당분간은 무조건 shared.json 재생성하게 수정
        ...
        if config.finetune:
            ...
            shared['word2idx'] = {word: idx + 4 for idx, word in
                                  enumerate(word for word, count in sorted(word_counter.items(), key=lambda x: x[1], reverse=True)
                                            if count > config.word_count_th and word not in word2vec_dict)}
        shared['char2idx'] = {char: idx + 2 for idx, char in
                              enumerate(char for char, count in sorted(char_counter.items(), key=lambda x: x[1], reverse=True)
                                        if count > config.char_count_th)}
        GO_TOKEN = "<GO>"
        EOS_TOKEN = "<EOS>"
        NULL = "-NULL-"
        UNK = "-UNK-"
        shared['word2idx'][GO_TOKEN] = 0
        shared['word2idx'][EOS_TOKEN] = 1
        shared['word2idx'][NULL] = 2
        shared['word2idx'][UNK] = 3

        shared['char2idx'][NULL] = 0
        shared['char2idx'][UNK] = 1
        print('word2idx : {}, char2idx : {}'.format(len(shared['word2idx']), len(shared['char2idx'])))

        """
        dev_shared_path = os.path.join(config.data_dir, "shared_{}.json".format("dev"))
        dev_shared = json.load(open(dev_shared_path, 'r'))

        # train + dev 단어에서 상위 3만개 중 glove에 있는 단어를 모아 (lower 옵션에따라,) word_emb_mat로 활용
        from collections import Counter
        qg_word_counter = Counter()
        qg_word_counter.update(word_counter)
        qg_word_counter.update(dev_shared['lower_word_counter'] if config.lower_word else dev_shared['word_counter'])
        qg_word2idx = {word: idx for idx, word in enumerate(elem for elem, cnt in qg_word_counter.most_common(30000))}
        # word2idx 74134, char2idx 281
        json.dump({'word2idx': shared['word2idx'],
                   'char2idx': shared['char2idx'],
                   'qg_word2idx': qg_word2idx}, open(shared_path, 'w'))
        exit(2017321214)
        """
        # all word2idx 70415, char2idx 281 / train+glove => word2idx , char2idx
        ...
        for key, val in new_shared.items():
            shared[key] = val
    # dict_keys(['char2idx', 'word2idx'])
    #print(new_shared.keys())
    # dict_keys(['lower_word2vec', 'char2idx', 'x', 'word2idx', 'cx', 'p', 'char_counter', 'lower_word_counter', 'word2vec', 'word_counter'])
    #print(shared.keys())

    if config.use_glove_for_unk:
        # create new word2idx and word2vec
        train_shared_path = os.path.join(config.data_dir, "shared_{}.json".format("train"))
        dev_shared_path = os.path.join(config.data_dir, "shared_{}.json".format("dev"))
        train_shared = json.load(open(train_shared_path))
        dev_shared = json.load(open(dev_shared_path))
        #word2vec_dict = shared['lower_word2vec'] if config.lower_word else shared['word2vec']

        word2vec_dict = train_shared['lower_word2vec'] if config.lower_word else train_shared['word2vec']
        word2vec_dict.update(dev_shared['lower_word2vec'] if config.lower_word else dev_shared['word2vec'])

        new_word2idx_dict = {word: idx for idx, word in enumerate(word for word in sorted(list(word2vec_dict.keys())) if word not in shared['word2idx'])}

        shared['new_word2idx'] = new_word2idx_dict
        idx2vec_dict = {idx: word2vec_dict[word] for word, idx in sorted(new_word2idx_dict.items(), key=lambda x: x[1])}
        print("{}/{} unique words have corresponding glove vectors.".format(len(idx2vec_dict), len(new_word2idx_dict)))
        new_emb_mat = np.array([idx2vec_dict[idx] for idx in range(len(idx2vec_dict))], dtype='float32')
        shared['new_emb_mat'] = new_emb_mat

        """
        qg_idx2vec_dict = {idx: word2vec_dict[word] for word, idx in sorted(shared['qg_word2idx'].items(), key=lambda x: x[1])}
        qg_emb_mat = np.array([qg_idx2vec_dict[idx] for idx in range(len(qg_idx2vec_dict))], dtype='float32')
        shared['qg_emb_mat'] = qg_emb_mat
        """
